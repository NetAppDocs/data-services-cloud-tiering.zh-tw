...
summary: 透過將非活動資料分層到NetApp Cloud Tiering 中的StorageGRID ，釋放本地ONTAP叢集上的空間。 
---
= 將資料從本地ONTAP叢集分層到NetApp Cloud Tiering 中的StorageGRID
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
透過將非活動資料分層到NetApp Cloud Tiering 中的StorageGRID ，釋放本地ONTAP叢集上的空間。
...
* 已新增至NetApp控制台的運行ONTAP 9.4 或更高版本的來源本機ONTAP集群，以及透過使用者指定連接埠到StorageGRID 的連線。 https://docs.netapp.com/us-en/bluexp-ontap-onprem/task-discovering-ontap.html["了解如何發現集群"^] 。
...
[role="quick-margin-para"]
在NetApp控制台中，選擇一個本機系統，選擇「啟用」雲端分層，然後依照指示將資料分層到StorageGRID。
...
授權:: 將資料分層到StorageGRID時，您的NetApp Console 組織不需要 Cloud Tiering 許可證， ONTAP叢集上也不需要FabricPool許可證。
叢集網路需求::
+
--
...
. 點擊右側面板中的“啟用雲端分層”****。
+
如果StorageGRID分層目標是作為NetApp控制台中的系統存在，則可以將叢集拖曳到StorageGRID系統上以啟動設定精靈。
