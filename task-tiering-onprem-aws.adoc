...
如果您希望透過網際網路從本機資料中心到 VPC 建立更安全的連接，則可以在分層啟動精靈中選擇 AWS PrivateLink 連接。如果您打算使用 VPN 或 AWS Direct Connect 透過使用私人 IP 位址的 VPC 終端節點介面連接您的本機系統，則需要它。這種連接類型顯示在<<連線選項的網路圖,上面的第二張圖>>。
